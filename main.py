...
class PushRecordManager:
    ...
    def has_pushed_today(self) -> bool:
        ...
        try:
            ...
            print(f"Failed to read push records: {e}")
            return False
...
def save_titles_to_file(results: Dict, id_to_name: Dict, failed_ids: List) -> str:
    ...
    with open(file_path, "w", encoding="utf-8") as f:
        ...
        if failed_ids:
            f.write("==== The following IDs failed ====\n")
...
def count_word_frequency(
    ...
    for source_id, titles_data in results_to_process.items():
        ...
        for title, title_data in titles_data.items():
            ...
            for group in word_groups:
                ...
                # If in "All News" mode, all titles match the first (only) word group
                if len(word_groups) == 1 and word_groups[0]["group_key"] == "All News":
                    group_key = group["group_key"]
    ...
    if mode == "incremental":
        if is_first_today:
            ...
            print(
                f"Incremental mode: first crawl today, {matched_new_count} out of {total_input_news} news items {filter_status}"
            )
        else:
            if new_titles:
                ...
                if matched_new_count == 0 and len(word_groups) > 1:
                    print("Incremental mode: no new news matches frequency words, will not send notification")
            else:
        ...
        if is_first_today:
            ...
            print(
                f"Current ranking mode: first crawl today, {matched_new_count} out of {total_input_news} current ranking news items {filter_status}"
            )
        else:
...
def generate_html_report(
    ...
    if is_daily_summary:
        if mode == "current":
            ...
            filename = "Incremental Update.html"
        else:
        ...
        <div class="container">
            <div class="header">
                <div class="save-buttons">
                    ...
                    <button class="save-btn" onclick="saveAsMultipleImages()">Save in Sections</button>
                </div>
    ...
    if report_data["new_titles"]:
        html += f"""
                <div class="new-section">
                    <div class="new-section-title">New Hot Topics (Total {report_data['total_new_count']} items)</div>"""
        ...
        <script>
            async function saveAsImage() {
                ...
                try {
                    ...
                    const now = new Date();
                    const filename = `TrendRadar_HotNewsAnalysis_${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}.png`;
            ...
            async function saveAsMultipleImages() {
                ...
                try {
                    ...
                    // 下载所有图片
                    const now = new Date();
                    const baseFilename = `TrendRadar_HotNewsAnalysis_${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}`;
                    ...
                    console.error('Failed to save in sections:', error);
                    const buttons = document.querySelector('.save-buttons');
                    buttons.style.visibility = 'visible';
                    button.textContent = 'Save failed';
                    setTimeout(() => {
...
def render_feishu_content(
    ...
    if report_data["new_titles"]:
        ...
        text_content += (
            f"🆕 **New Hot Topics** (Total {report_data['total_new_count']} items)\n\n"
        )
...
def render_dingtalk_content(
    ...
    text_content += f"**Type:** Hot News Analysis Report\n\n"
...
def split_content_into_batches(
    ...
    if format_type == "wework":
        ...
        base_header += f"**Type:** Hot News Analysis Report\n\n"
        base_header += "---\n\n"
...
def send_to_dingtalk(
    ...
    for i, batch_content in enumerate(batches, 1):
        ...
        payload = {
            "msgtype": "markdown",
            "markdown": {
                "title": f"TrendRadar Hot News Analysis Report - {report_type}",
                "text": batch_content,
            },
        }
...
Please use an HTML-enabled email client to view the full report content.
        """
        ...
        try:
            ...
            print(f"Email sending failed: server unexpectedly disconnected, please check network or try again later")
            return False
...
def send_to_ntfy(
    ...
    print(f"ntfy will push in reverse order (last batch first) to ensure correct display order in client")
    ...
    for idx, batch_content in enumerate(reversed_batches, 1):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"ntfy batch {actual_batch_num}/{total_batches} message too large rejected [{report_type}], message size: {batch_size} bytes"
                )
            else:
                print(
            ...
            print(f"ntfy batch {actual_batch_num}/{total_batches} send error [{report_type}]: {e}")
    ...
    if success_count == total_batches:
        ...
        print(f"ntfy partially sent successfully: {success_count}/{total_batches} batches [{report_type}]")
...
class NewsAnalyzer:
    ...
    MODE_STRATEGIES = {
        "incremental": {
            ...
            "description": "Incremental mode (only focus on new news, no push when no new items)",
    ...
    def _has_valid_content(
        ...
        if self.report_mode in ["incremental", "current"]:
            ...
            # In daily summary mode, check if there are matched frequency word news or new news
    ...
    def _send_notification_if_needed(
        ...
        if (
            ...
            if "Realtime" in report_type:
                print(
                    f"Skipping realtime push notification: no matching news detected in {mode_strategy['mode_name']}"
                )
            else:
                print(
    ...
    def _initialize_and_check_config(self) -> None:
        ...
        if not CONFIG["ENABLE_NOTIFICATION"]:
            print("Notification feature disabled (ENABLE_NOTIFICATION=False), will only perform data crawling")
        elif not has_notification:
            print("No notification channels configured, will only perform data crawling, no notifications will be sent")
        else:
            print("Notification feature enabled, will send notifications")
