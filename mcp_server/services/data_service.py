...
Provides unified data query interface, encapsulates data access logic.
"""
...
class DataService:
    ...
    def get_trending_topics(
        ...
        rather than automatically extracting hot topics from news. Users can customize this keywords list.
        ...
        if mode == "daily":
            ...
            # current mode: only process latest batch data (file with latest timestamp)
            if timestamps:
                # Find latest timestamp
                latest_timestamp = max(timestamps.values())

                # Re-read, only get latest time data
                # Here we reverse lookup the platform corresponding to the latest file through timestamps dictionary
                latest_titles, _, _ = self.parser.read_all_titles_for_date()

                # Since read_all_titles_for_date returns merged data from all files,
                # we need to filter out the latest batch through timestamps
    ...
    def get_available_date_range(self) -> Tuple[Optional[datetime], Optional[datetime]]:
        """
        Scan output directory, return actual available date range

        Returns:
            (earliest date, latest date) tuple, returns (None, None) if no data
