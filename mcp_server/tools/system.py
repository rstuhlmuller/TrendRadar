...
Implements system status query and crawler trigger functionality.
"""
...
class SystemManagementTools:
    ...
    def trigger_crawl(self, platforms: Optional[List[str]] = None, save_to_local: bool = False, include_url: bool = False) -> Dict:
        ...
        Args:
            platforms: Specify platform list, if empty then crawl all platforms
        ...
        Returns:
            Crawl result dictionary, containing news data and save path (if saved)

        Example:
            >>> tools = SystemManagementTools()
            >>> # Temporary crawl, don't save
            >>> result = tools.trigger_crawl(platforms=['cnn', 'bbc'])
            >>> print(result['data'])
            >>> # Crawl and save to local
            >>> result = tools.trigger_crawl(platforms=['cnn'], save_to_local=True)
        ...
        try:
            ...
            if not config_path.exists():
                raise CrawlTaskError(
                    ...
                    suggestion=f"Please ensure configuration file exists: {config_path}"
                )
            ...
            if not all_platforms:
                raise CrawlTaskError(
                    "No platform configuration in config file",
                    suggestion="Please check platforms configuration in config/config.yaml"
                )
            ...
            # If persistence is needed, call save logic
            if save_to_local:
                try:
                    import re
                    ...
                    result["note"] = "Crawl successful but save failed, data only in memory"
            else:
